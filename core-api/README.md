...
Assets are aggregate documents. The asset row stores its nested content in the `content` JSONB column, including prototypes, animations, dynamic animation directions, frames, and their generation states. Asset reads and writes use the asset as the consistency boundary. Asset resources are not modeled as a separate table.
